...
namespace LoveHue
{
    class HueWrapper : IEnumerable<HueWrapper.HueLamp>
    {
        private static void SetValue(HttpClient httpClient, string Header, string name, object value)
        {
            ...
            if (obj["error"] != null)
                Debug.WriteLine($"ERROR: {obj["error"]["description"]}");
        }
            ...
            try
            {
                ...
                if (username == null)
                {
                    do
                    {
                        if (idJObject != null)
                        {
                            Debug.WriteLine($"ERROR: {idJObject["error"]["description"]}");
                            Thread.Sleep(100);
                        }
                ...
                Debug.WriteLine($"CONNECTION ESTEBLISHED: {hueWrapper.username}");
                ...
                {
                    Debug.WriteLine(hueWrapperLamp.ToString());
                }
